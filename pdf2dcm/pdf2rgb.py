...
class Pdf2RgbSC(BaseConverter):
    ...
    def run(
        ...
        for idx, page in enumerate(pages):
            file_name_out =  f"{output_prefix}{path_pdf_path.stem}-{idx+1:05d}{suffix}"
            file_name_out = file_name_out.replace(' ', '_') # remove spaces in name. 
            store_dcm_path = Path(os.path.join(output_path, file_name_out))
